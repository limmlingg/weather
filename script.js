...
function getWeather() {
	$.getJSON("http://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lon + "&appid=f0226db01036e625782a08842bf20938", function(data) {
		// console.log(data);
...
function getMap() {
	...
	$.getJSON("https://maps.googleapis.com/maps/api/streetview/metadata?size=" + height + "x" + width + "&location=" + lat + "," + lon + "&key=AIzaSyAAr7n_meQxdm_sC6wta1hFIm8gaYEkz_Y", function(d) {
		//console.log(d.status);
		if (d.status=="OK") { // Update background image if Streetview image is found
